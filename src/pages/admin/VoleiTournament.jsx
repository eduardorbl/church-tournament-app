...
export default function VoleiTournament() {
  ...
  // Carrega esporte + times e reconstrói grupos respeitando seed_in_group quando existir
  const loadAll = async () => {
    ...
    try {
      // 1) Modalidade - usar "Volei" (sem acento) para bater com o banco
      ...
      // Pré-preenche grupos com base no que já existe (ordenando por seed_in_group 1..3)
      const next = { A: [], B: [], C: [] };
      const bucket = { A: [], B: [], C: [] };
      (tms || []).forEach((t) => {
        if (t.group_name && GROUP_KEYS.includes(t.group_name)) {
          bucket[t.group_name].push(t);
        }
      });
      GROUP_KEYS.forEach((g) => {
        const sorted = (bucket[g] || []).sort((a, b) => {
          const sa = a.seed_in_group ?? 999;
          const sb = b.seed_in_group ?? 999;
          if (sa !== sb) return sa - sb;
          return a.name.localeCompare(b.name);
        });
        next[g] = sorted.map((t) => t.id);
      });
      setGroups(next);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    // Opcional: atualizar se outro admin mexer nos times/grupos
    const ch = supabase
      .channel("volei-admin")
  ...
  // ====== Confirmar grupos → salvar group_name + seed e gerar jogos via RPC ======
    ...
    <div className="space-y-8">
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Organize os 9 times em 3 grupos de 3. Ao confirmar, salvamos{" "}
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          ...
          disabled={!GROUP_KEYS.every((g) => groups[g].length === TEAMS_PER_GROUP) || busy}
          className={`px-6 py-2 rounded font-semibold transition ${
            GROUP_KEYS.every((g) => groups[g].length === TEAMS_PER_GROUP) && !busy
